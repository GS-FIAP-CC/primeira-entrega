...
"""Aplica regressão linear (método dos mínimos quadrados) para estimar o comportamento futuro da reserva de energia. 
Também oferece a média móvel como técnica de comparação."""

# Calcula a reta de melhor ajuste (y = a*x + b) pelo método dos mínimos quadrados, sendo a quantidade de pontos de dados.
def regressao_linear(xs, ys):
    ...
    # Soma de todos os Y
    soma_y  = sum(ys)  
    # Soma dos produtos X*Y
    soma_xy = sum(xs[i] * ys[i] for i in range(n))      
    # Soma dos X ao quadrado
...
def media_movel(valores, janela=3):
    # Pega os últimos N valores
    ultimos_valores = valores[-janela:]              
